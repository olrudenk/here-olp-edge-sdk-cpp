...
curl_proxytype to_curl_proxy_type(olp::network2::NetworkProxySettings::Type type) {
  using ProxyType = olp::network2::NetworkProxySettings::Type;
  switch (type) {
    case ProxyType::HTTP:
...
void NetworkProtocolCurl::Teardown() {
  ...
  // handles teardown
  std::vector<std::pair<RequestId, Network::Callback> >
      completedMessages;
  {
    std::lock_guard<std::mutex> lock(event_mutex_);
    events_.clear();
  ...
  if (!completedMessages.empty()) {
    for (auto& pair : completedMessages) {
      pair.second(network2::NetworkResponse()
...
NetworkStatus NetworkProtocolCurl::Send(NetworkRequest request,
                                        std::shared_ptr<std::ostream> payload,
                                        Network::Callback callback,
                                        Network::HeaderCallback header_callback,
                                        Network::DataCallback data_callback){

  RequestId request_id {static_cast<RequestId>(RequestIdConstants::RequestIdMin)};
  {
    std::lock_guard<std::mutex> lock(event_mutex_);

    request_id = next_request_id_;
    if (next_request_id_ == static_cast<RequestId>(RequestIdConstants::RequestIdMax)) {
      next_request_id_ = static_cast<RequestId>(RequestIdConstants::RequestIdMin);
    } else {
      next_request_id_++;
    }
  }

  auto error_status = Send2(request, request_id, payload, std::move(header_callback), 
                            std::move(data_callback), std::move(callback));

  if (error_status == ErrorCode::NO_ERROR) {
    return NetworkStatus(request_id);
  }

  return NetworkStatus(error_status);
}


ErrorCode NetworkProtocolCurl::Send2(
  ...
  if (proxy.GetType() != NetworkProxySettings::Type::NONE) {
    curl_easy_setopt(handle->handle, CURLOPT_PROXY, proxy.GetHostname().c_str());
    curl_easy_setopt(handle->handle, CURLOPT_PROXYPORT, proxy.GetPort());
    const auto proxyType = proxy.GetType();
    if (proxyType != NetworkProxySettings::Type::HTTP) {
    ...
    assert(proxy.GetUsername().empty() == proxy.GetPassword().empty());
  ...
  if (!curlCaBundle.empty()) {
    ...
    if (CURLE_OK != error) {
      return ErrorCode::UNKNOWN_ERROR;//NetworkProtocol::ErrorFailedSetCaCert;
    }
    EDGE_SDK_LOG_TRACE(kLogTag, "curl bundle path: " << curlCaBundle);
  }
#endif
...
void NetworkProtocolCurl::Cancel(RequestId id) {
...
size_t NetworkProtocolCurl::RxFunction(void* ptr, size_t size, size_t nmemb,
  ...
  if (that->IsStarted() && !handle->range_out && !handle->cancelled) {
    if (handle->data_callback) {
      handle->data_callback(reinterpret_cast<uint8_t*>(ptr), handle->offset + handle->count, len);
    }
    if (handle->payload) {
    ...
    if ((result == CURLE_OK) || (result == CURLE_HTTP_RETURNED_ERROR)) {
      ...
      if (result == CURLE_PARTIAL_FILE) {
        ...
        if (code == CURLE_OK &&
          ...
          status = static_cast<int>(ErrorCode::TIMEOUT_ERROR);//Network::TimedOut;
        }
      }
    }
    ...
    CURLMsg* msg(nullptr);
    {
      ...
      while (IsStarted() && (msg = curl_multi_info_read(curl_, &left))) {
        ...
        if (msg->msg == CURLMSG_DONE) {
          ...
          EDGE_SDK_LOG_ERROR(kLogTag, "Message complete with unknown state " << msg->msg);
      ...
      if (!lostHandles.empty() && IsStarted()) {
        ...
        for (CURL* handle : lostHandles) {
          ...
          if (remove_handle_status == CURLM_OK) {
            ...
            EDGE_SDK_LOG_ERROR(kLogTag, "lost handle curl_multi_remove_handle error %s "
