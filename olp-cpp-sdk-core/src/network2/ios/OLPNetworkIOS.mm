...
constexpr olp::network2::RequestId kInvalidRequestId =
    static_cast<RequestId>(RequestIdConstants::RequestIdInvalid);

const char* kLogTag = "OLPNetworkIOS";

NSMutableDictionary* ParseHeadersDictionaryFromRequest(
    const olp::network2::NetworkRequest& request) {
...
NSData* ParseBodyDataFromRequest(const olp::network2::NetworkRequest& request) {
...
olp::network2::ErrorCode ConvertNSURLErrorToNetworkErrorCode(
...
olp::network2::NetworkStatus OLPNetworkIOS::Send(
    olp::network2::NetworkRequest request,
    std::shared_ptr<std::ostream> payload,
    olp::network2::Network::Callback callback,
    olp::network2::Network::HeaderCallback header_callback,
    olp::network2::Network::DataCallback data_callback) {
  @autoreleasepool {
    ...
    // create new OLPHttpTask, if possible
    {
      ...
      if (http_client_.activeTasks.count >= max_requests_count_) {
        EDGE_SDK_LOG_WARNING_F(
            kLogTag,
            "Can't send request:[%s] - reached max requests count:[%lu / %lu]!",
            request.GetUrl().c_str(), http_client_.activeTasks.count,
            max_requests_count_);
        return NetworkStatus(ErrorCode::NETWORK_OVERHEAD_ERROR);
      }

      olp::network2::RequestId request_id = GenerateNextRequestId();
    ...
    if (!task) {
      ...
      return NetworkStatus(ErrorCode::UNKNOWN_ERROR);
    }
    ...
    task.completionHandler = ^(NSError* error) {
      ...
      int status = static_cast<int>(ErrorCode::NO_ERROR);
      ...
      if (callback) {
        ...
        callback(olp::network2::NetworkResponse()
    ...
    if (OLPHttpTaskStatusOk != ret) {
      ...
      return NetworkStatus(kInvalidRequestId);
    }
    return NetworkStatus(task.requestId);
  }  // autoreleasepool
}
...
void OLPNetworkIOS::Cancel(olp::network2::RequestId identifier) {
  std::lock_guard<std::mutex> lock(mutex_);
  @autoreleasepool {
...
olp::network2::RequestId OLPNetworkIOS::GenerateNextRequestId() {
