...
- (instancetype)init {
  ...
  if (self) {
    ...
    _sharedUrlSession =
        [self urlSessionWithProxy:olp::network2::NetworkProxySettings()
                       andHeaders:nil];
...
- (OLPHttpTask*)createTaskWithProxy:
  ...
  BOOL isProxyValid = proxySettings.GetType() !=
                          olp::network2::NetworkProxySettings::Type::NONE &&
...
- (NSURLSession*)urlSessionWithProxy:
  ...
  if (isProxyValid) {
    ...
    if (proxyName.length) {
      ...
      BOOL httpProxy = YES;
      if (olp::network2::NetworkProxySettings::Type::SOCKS4 ==
              proxySettings.GetType() ||
          olp::network2::NetworkProxySettings::Type::SOCKS5 ==
              proxySettings.GetType() ||
          olp::network2::NetworkProxySettings::Type::SOCKS5_HOSTNAME ==
              proxySettings.GetType()) {
